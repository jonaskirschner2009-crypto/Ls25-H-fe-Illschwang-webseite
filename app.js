                    ...
                    <div>
                        <div class="hof-card-header-v31 mb-3">
                            <div class="hof-card-title-actions-v31">
                                <div class="hof-card-name-v32 min-w-0">
                                ...
                                <div class="hof-card-actions-v31" aria-label="Hof-Aktionen">
                                    <button type="button" onclick="toggleFavorite(event, ${hof.id})" title="${favoriteHoefe.includes(hof.id)?'Favorit entfernen':'Zu Favoriten hinzufügen'}" aria-label="${favoriteHoefe.includes(hof.id)?'Favorit entfernen':'Zu Favoriten hinzufügen'}" class="favorite-btn ${favoriteHoefe.includes(hof.id)?'favorite-active':''}">${favoriteHoefe.includes(hof.id)?'♥':'♡'}</button>
                                    <button type="button" onclick="toggleCompare(event, ${hof.id})" title="Hof vergleichen" aria-label="Hof vergleichen" class="hof-compare-action ${compareHoefe.includes(hof.id)?'is-active':''}">${compareHoefe.includes(hof.id)?'✓':'⚖️'}</button>
                                </div>
                            </div>
                            <div class="hof-card-status-v31">
                                ${isAdmin ? `<button onclick="toggleStatus(event, ${hof.id})" class="px-3.5 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider transition shadow-sm cursor-pointer whitespace-nowrap ${isVerkauft ? 'badge-verkauft hover:bg-red-700' : 'badge-zu-verkaufen hover:bg-emerald-600'}">${isVerkauft ? 'Verkauft' : 'Zu verkaufen'}</button>` : `<span class="px-3.5 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider whitespace-nowrap ${isVerkauft ? 'badge-verkauft' : 'badge-zu-verkaufen'}">${isVerkauft ? 'Verkauft' : 'Zu verkaufen'}</span>`}
                            </div>
                        </div>
